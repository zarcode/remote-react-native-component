...
const HomePage = ({navigation}) => {
  ...
  useEffect(() => {
    const id = 'counter';
    let code = fetchComponent(id);
    dispatch(addComponent({id, code}));
  }, []);

  const todos = useSelector(state => state.todos);
  const dispatch = useDispatch();
  const addTodoToStore = () =>
    todoText.trim() && dispatch(addTodo({title: todoText}));
  const toggleTodoToStore = (id, completed) =>
    ...
    <>
      {showDynamicComponents && (
        <View style={{paddingVertical: 15}}>
          <DynamicComponentFromState __id="counter" />
          <View
            style={{
              flexDirection: 'row',
              justifyContent: 'space-between',
      ...
      <View>
        {todos.map(todo => (
          <TouchableOpacity
            ...
            <Icon
              name={
                todo.completed ? 'checkbox-marked' : 'checkbox-blank-outline'
              }
              size={20}
              color="#3480eb"
            />
